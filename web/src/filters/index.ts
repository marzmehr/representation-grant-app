...
Vue.filter("beautify-date-weekday", function(date) {
  if (date) return moment(date).format("ddd MMM DD, YYYY HH:mm");
  else return "";
});
...
Vue.filter("getFullName", function(nameObject) {
  if (nameObject) {
    return nameObject.first + " " + nameObject.middle + " " + nameObject.last;
  } else {
    return " ";
  }
});
...
Vue.filter("getSurveyResults", function(
  survey,
  currentStep: number,
  currentPage: number
) {
  ...
  for (const question of survey.currentPage.questions) {
    if (
      question.isVisible &&
      question.questionValue != true &&
      question.questionValue != false
    )
      if (question.name == "bankAccountInfoPanel") {
        ...
        for (const panel of question.panels) {
          ...
          for (const panelquestion of panel.questions) {
            ...
            if (
              panelquestion.isVisible &&
              panelquestion.questionValue != true &&
              panelquestion.questionValue != false
            )
              questionResults.push({
                name: panelquestion.name,
                value: panelquestion.questionValue,
                title: panelquestion.fullTitle,
                inputType: panelquestion.inputType
              });
          }
        }
  ...
  return {
    ...
    pageName: survey.currentPage.title,
