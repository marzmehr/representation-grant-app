...
import { formatAddressWithPostalCode, formatMonthDayYear, formPdfHtml, formPdfHtmlPGTLetter, convertBlobAndDownload } from "@/utils/utils";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const loadSurveyData = survey => {
      ...
      for (const successor of childSuccessors) {
        if (successor) {
          successors.push(successor);
        }
      }
    ...
    const onPrint = async () => {
      ...
      const html = formPdfHtmlPGTLetter(
        innerHTML,
