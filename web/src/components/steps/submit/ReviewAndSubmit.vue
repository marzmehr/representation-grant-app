<template>
  ...
  >
    ...
    <b-card
      style="borde:1px solid; border-radius:10px;"
      bg-variant="white"
      class="mt-4 mb-3"
    >
      <div class="ml-2">
        You have indicated that you will file at the following court registry:
        <p class="h4 mt-2 ml-2 mb-1" style="display:block">
      ...
      >
        ...
        <form-list
          id="formslist"
          type="Save"
          @formsList="setFormList"
          currentPage="-1"
        />

        <div
          class="my-4 text-primary"
          @click="showGetHelpForPDF = true"
          style="border-bottom:1px solid; width:20.25rem;"
        >
          <span style="font-size:1.2rem;" class="fa fa-question-circle" /> Get
          help opening and saving PDF forms
        </div>

        <div>
          Note: If you need to edit any of your answers, go back to the "Review
          Your Answers" page, edit the answer and return to this page.
        </div>
      </b-card>
      ...
      >
        <span class="text-primary" style="font-size:1.4rem;"
          >Additional Documents to Include:</span
        >

        <div>
          The following additional documents are required as part of your
          filing:
        </div>
        <ul class="mt-3">
          <li
      ...
      >
        ...
        <div class="ml-2">
          <ul class="mt-3">
            <li class="mb-2">
              Collect any existing orders or agreements, existing protection
              orders and any exhibits referenced in your application
            </li>
            <li>
              Scan and save an electronic copy of any existing orders or
              agreements, existing protection orders and any exhibits referenced
              in your application to your computer
            </li>
            <div
              class="my-3 text-primary"
              @click="showGetHelpScanning = true"
              style="border-bottom:1px solid; width:15.7rem;"
            >
              <span style="font-size:1.2rem;" class="fa fa-question-circle" />
              Get help scanning documents
            </div>
        ...
        <b-card id="drop-area" @click="uploadClicked">
          <div style="padding:0; margin: 0 auto; width:33px;">
            <label class="btn btn-default btnfile">
              <span
                style="font-size:20px; transform:translate(0,-17px);"
                class="fa fa-plus"
              ></span>
              <input
                id="inputfile"
          ...
          <p class="text-center m-0 text-info">
            Drag and Drop the PDF documents or JPG/PNG images here,
          </p>
          <p class="text-center m-0 text-info" style="">
            or click here to Browse for files
          </p>
        </b-card>

        <b-card border-variant="white" body-class="mb-0 pb-0">
          <h4 style="height: 2px; padding:0; margin:0 0 1.5rem 0;">
            Uploaded Documents:
          </h4>
      ...
      >
        <span class="text-primary" style="font-size:1.4rem;"
          >Filing with Court Services Online:</span
        >

        <div class="mt-3">
          <p>
            When you click Submit Documents, you will be taken to the Court
            Services Online e-filing hub. In the next few steps you will be able
            to do a final review of the documents you are submitting for filing
            and (if completed successfully) receive a Package Number.
          </p>
          <p>
            Once your filings have been reviewed by the Court Registry, you will
            be provided a Court File Number via e-mail. This may take up to one
            week.
          </p>
          <p style="font-weight: bold;">
            You will need your Court File Number if you are filing any
            additional documentation.
          </p>
        </div>

        <div style="width:19rem; margin: 0 auto;">
          <b-button v-on:click.prevent="onSubmit()" variant="success">
    ...
    <b-modal size="xl" v-model="showGetHelpForPDF" header-class="bg-white">
      ...
      <template v-slot:modal-footer>
        <b-button variant="primary" @click="showGetHelpForPDF = false"
          >Close</b-button
        >
      </template>
    ...
    <b-modal size="xl" v-model="showGetHelpScanning" header-class="bg-white">
      ...
      <get-help-scanning />
      <template v-slot:modal-footer>
        <b-button variant="primary" @click="showGetHelpScanning = false"
          >Close</b-button
        >
      </template>
      <template v-slot:modal-header-close>
        <b-button
          variant="outline-dark"
          class="closeButton"
          @click="showGetHelpScanning = false"
          >&times;</b-button
        >
      </template>
    </b-modal>
    ...
    >
      <b-card style="border-radius:10px" class="bg-light">
        <h1
          ...
          style="border-radius:10px; width:35rem; margin:0 auto; padding:1rem 0;"
        >
          Specify the Type of Document
        </h1>
        <div v-if="supportingFile" class="h3 my-5 text-center">
          File Name:
          <span class=" h2 text-danger"> {{ supportingFile.name }} </span>
        </div>
        <b-form-group style="width:30rem; margin: 4rem auto;">
          ...
          >
            <b-form-select-option
              ...
              :value="docType.value"
              :key="docType.value"
              >{{ docType.text }}</b-form-select-option
            >
          </b-form-select>
        </b-form-group>
      </b-card>
      <template v-slot:modal-footer>
        <b-button
          ...
          variant="primary"
          @click="showTypeOfDocuments = false"
          >Cancel</b-button
        >
        <b-button
...
export default class ReviewAndSubmit extends Vue {
  ...
  mounted() {
    ...
    if (location == "Victoria") {
      this.applicantLocation = {
        ...
        cityStatePostcode: "Victoria, B.C.  V8W 9J2",
        email: "Victoria.CourtScheduling@gov.bc.ca"
      };
    } else if (location == "Surrey") {
      this.applicantLocation = {
        name: "Surrey Provincial Court",
        address: "14340 - 57 Avenue",
        cityStatePostcode: "Surrey, B.C.  V3X 1B2",
        email: "CSBSurreyProvincialCourt.FamilyRegistry@gov.bc.ca"
      };
    }
    //TODO: use get api for document-types
    this.fileTypes = this.documentTypesJson;
    this.requiredDocuments = Vue.filter("extractRequiredDocuments")(
      ...
      if (step.active) {
        for (const page of step.pages) {
          if (
            page.active &&
            page.progress != 100 &&
            optionalLabels.indexOf(page.label) == -1
          ) {
            this.UpdateCurrentStep(step.id);
            this.UpdateCurrentStepPage({
              currentStep: step.id,
  ...
  public getRepGrantResultData() {
    const result = this.steps[0].result;
    for (let i = 1; i < 9; i++) {
    ...
    const deceasedDateOfDeath = {
      deceasedDateOfDeath: this.deceasedDateOfDeath
    };
  ...
  public eFile() {
    ...
    for (const index in this.supportingDocuments) {
      ...
      docType.push({
        type: supportingDoc["documentType"],
        files: [Number(index)],
        rotations: [supportingDoc["imageRotation"]]
      });
    ...
    this.$http.post(url, bodyFormData, header).then(
      res => {
        console.log(res);
