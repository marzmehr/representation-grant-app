    ...
    >
      ...
      <ol style="margin:0rem 0 0 -1.5rem;">
        ...
        <li class="mt-4">
          <div
            v-if="applicantList.length > 1"
            style="margin:0 0 .25rem 0; display:inline-block;"
          >
            We have made a diligent search and inquiry to find the property and
            liabilities of the deceased.
          </div>
          <div v-else style="margin:0 0 .25rem 0; display:inline-block;">
            I have made a diligent search and inquiry to find the property and
            liabilities of the deceased.
          </div>
        </li>
        <!-- <3> -->
        <li class="mt-4">
          <div style="display:inline;">
            Attached to this affidavit as Exhibit A is a Statement of Assets,
            Liabilities and Distribution that discloses
            <ol style="margin-left: -1.25rem;" class="resetcounter">
              <li class="bracketalpha">
                all of the property of the deceased, irrespective of its
        ...
        <li class="mt-3">
          <div v-if="applicantList.length > 1" style="display:inline;">
            If we determine that there is any property or liability that has not
            been disclosed in Exhibit A, or that information contained in this
            affidavit is incorrect or incomplete, we will promptly after
          ...
          <div v-else style="display:inline;">
            If I determine that there is any property or liability that has not
            been disclosed in Exhibit A, or that information contained in this
            affidavit is incorrect or incomplete, I will promptly after learning
            of the same file an affidavit of assets and liabilities in Form P14
            to disclose the correct and complete information.
          </div>
        </li>
        ...
        <li class="mt-3">
          <div v-if="applicantList.length > 1" style="display:inline;">
            In addition to the probate fees payable in relation to any property
            disclosed in Exhibit A, we promise to pay the Minister of Finance
            the probate fees payable with respect to the value of any property
            that passes to us as the deceased's personal representative, and
            that is not disclosed in Exhibit A, on a determination being made as
            to the value of that asset.
          </div>
          <div v-else style="display:inline;">
            In addition to the probate fees payable in relation to any property
            disclosed in Exhibit A, I promise to pay the Minister of Finance the
            probate fees payable with respect to the value of any property that
            passes to me as the deceased's personal representative, and that is
            not disclosed in Exhibit A, on a determination being made as to the
            value of that asset.
          </div>
        </li>
      </ol>
      ...
      <div
        style="text-align:center;margin:1rem 0 0 0;font-weight:bold;font-size:14pt;"
      >
      ...
      <div style="margin-top:1rem;">
          ...
          <template v-slot:head(part)>
            ...
            <div>
              ...
              <span style="font-weight:normal;"
                >(including mortgages and vendors' and purchasers' interests in
                agreements for sale)</span
              >
            </div>
          </template>
          <template v-slot:cell(part)="data">
            <div
              v-if="data.value == 'TOTAL'"
        ...
        <b-table
          :fields="personalTangibleFields"
          :items="personalTangibleItems"
          small
        >
          <template v-slot:head(part)>
        ...
        <b-table
          :fields="personalIntangibleFields"
          :items="personalIntangibleItems"
          small
        >
          <template v-slot:head(part)>
            ...
            <div>
              ...
              <span style="font-weight:normal;"
                >(including bank accounts, intellectual property and other
                valuable items that cannot be touched by hand)</span
              >
            </div>
          </template>
          <template v-slot:cell(part)="data">
...
export default class FormP10 extends Vue {
  ...
  public getBelongingsInfo() {
    ...
    if (this.steps[5] && this.steps[5].result) {
      ...
      if (
        belongingsInfo["vehiclesSurvey"] &&
        belongingsInfo["vehiclesSurvey"].data
      ) {
        const vehiclesSurvey = belongingsInfo["vehiclesSurvey"].data;
        if (
          vehiclesSurvey.vehicleExists &&
          vehiclesSurvey.vehicleExists == "y"
        ) {
      ...
      if (
        belongingsInfo["bankAccountsSurvey"] &&
        belongingsInfo["bankAccountsSurvey"].data
      ) {
            ...
            if (bankInfo.accountPanel && bankInfo.accountPanel.length > 0) {
              ...
              for (const accountIndex in bankInfo.accountPanel) {
                ...
                if (
                  ...
                  accountInfo.accountValue == "other"
                ) {
                  if (accountInfo.accountValueComment) {
                    valueRow =
                      ...
                      Vue.filter("currencyFormat")(
                        accountInfo.accountValueComment
                      );
                    totalValue =
                      totalValue + Number(accountInfo.accountValueComment);
                  }
                } else if (
                  accountInfo.accountValue &&
                  ...
                  if (
                    ...
                    this.steps[7].result["finalizeAssetValuesSurvey"].data[
                      "finalizeAssetPlaceholder"
                    ]
                  ) {
                    const accountValue = this.steps[7].result[
                      "finalizeAssetValuesSurvey"
                    ].data["finalizeAssetPlaceholder"];
                    const key =
                      'Account Number "' + accountNumber + '" at ' + bankName;
                    // console.log(key)
                    ...
                    if (accountValue[key] && accountValue[key].length > 0) {
                      valueRow =
                        valueRow +
                        "<br>" +
                        Vue.filter("currencyFormat")(accountValue[key]);
