<template>
    <page-base v-bind:hideNavButtons="!showTable || !showOrgTable" v-bind:disableNext="isDisableNext()" v-on:onPrev="onPrev()" v-on:onNext="onNext()" >
        <div class="home-content">
            ...
            <b-card v-if="creditorPersonExists== 'Yes' && (creditorData.length > 0) && incompleteError && showTable" name="incomplete-error" class="alert-danger p-3 my-4 " no-body :key="updated+1">
            ...
            <b-card v-if="(creditorPersonExists== 'Yes' && (creditorData.length > 0) && !incompleteError && showTable) || (creditorOrgExists== 'Yes' && (creditorOrgData.length > 0) && !incompleteOrgError && showOrgTable)" 
                ...
                <b-row no-body class="info-box">
                    ...
                    <b-col cols="11" style="padding-left: 0 !important; margin-left: 0 !important;">
                        <p>
                            In this step, you have identified that {{deceasedName | getFullName}} may have owed more than $10,000 
                            to each the following people:                                        
                            <ul>
                        ...
                        <p>
                            In this step, you have identified that {{deceasedName | getFullName}} may have owed more than $10,000 
                            to each the following organizations:
                            <ul><b>{bullets(creditorOrganizationInfoPanel.creditorOrganizationName)}</b></ul>)}\n\n
...
export default class CreditorInfo extends Vue {
    ...
    public surveyHasError(){
        let progress = this.creditorData.length==0? 50 : 100;
        
        if(this.creditorPersonExists == 'No') progress =100;

        this.incompleteError =  false;        
        for(const creditor of this.creditorData){
            if (!creditor.creditorName || !creditor.creditorIsAlive){            
    ...
    beforeDestroy() {
        ...
        this.UpdateStepResultData({step:this.step, data: {creditorPersonExists: this.creditorPersonExists, creditorSurvey: creditorInfo}})       
    }
