(function() {
  ...
  function removeClass(elt, cls) {
    ...
    if (elt.classList) elt.classList.remove(cls);
      ...
      const cls = elt.className.split(/\s+/);
      const lst = cls.filter(function(a) {
        return a != cls;
      });
  ...
  TooltipMgr.prototype = {
    ...
    register: function(target, content, params) {
      if (!target) return;
      const tip = new Tooltip(target, content, params),
        skip = function(evt) {
          evt.preventDefault();
          evt.stopPropagation();
        },
  ...
  Tooltip.prototype = {
    init: function(target, content, params) {
      ...
      if (params) {
        if (params.extClass) this.extClass = params.extClass;
      }
    },
    render: function() {
      if (this.elt) return;
    ...
    position: function() {
      ...
      this.elt.style.top =
        "" + (tgt.top + tgt.height + window.pageYOffset) + "px";
      this.elt.style.left =
        "" +
        (tgt.left + tgt.width / 2 - sz.width / 2 + window.pageXOffset) +
        "px";
    },
    hide: function() {
      var elt = this.elt;
